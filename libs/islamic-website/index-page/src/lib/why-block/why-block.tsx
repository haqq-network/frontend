import { Container, Heading, Text } from '@haqq/islamic-ui-kit';
import { PropsWithChildren } from 'react';

function IslamStar() {
  return (
    <svg
      width="44"
      height="44"
      viewBox="0 0 44 44"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M28.4109 6.12028L28.5341 6.22526L28.6955 6.23814L37.0918 6.90817L37.7619 15.3045L37.7748 15.4659L37.8797 15.5891L43.3431 22L37.8797 28.4109L37.7748 28.5341L37.7619 28.6955L37.0918 37.0918L28.6955 37.7618L28.5341 37.7747L28.4109 37.8797L22 43.3431L15.5891 37.8797L15.4659 37.7747L15.3045 37.7618L6.90816 37.0918L6.23814 28.6955L6.22526 28.5341L6.12028 28.4109L0.656931 22L6.12028 15.5891L6.22526 15.4659L6.23814 15.3045L6.90816 6.90817L15.3045 6.23814L15.4659 6.22526L15.5891 6.12028L22 0.656931L28.4109 6.12028Z"
        stroke="#18FFAC"
      />
    </svg>
  );
}

function DoubleCrescent() {
  return (
    <svg
      width="44"
      height="43"
      viewBox="0 0 44 43"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M36.5863 9.5C39.1078 9.5 41.4475 10.2661 43.3839 11.5763C37.867 11.6433 33.4098 16.1066 33.4098 21.6098C33.4098 27.0054 37.6944 31.4014 43.0615 31.6344C41.1901 32.8157 38.9695 33.5 36.5863 33.5C29.908 33.5 24.5 28.1242 24.5 21.5C24.5 14.8758 29.908 9.5 36.5863 9.5Z"
        stroke="#18FFAC"
      />
      <path
        d="M34.2789 38.7683L34.2789 38.7683C30.8021 41.1208 26.5803 42.5 22.0261 42.5C10.1261 42.5 0.5 33.0867 0.5 21.5001C0.5 9.91333 10.1261 0.5 22.0261 0.5C26.6388 0.5 30.9105 1.91486 34.4125 4.32282L34.6958 3.91081L34.4125 4.32283C34.4711 4.36314 34.4884 4.40082 34.4952 4.42817C34.5035 4.46151 34.5028 4.51088 34.4783 4.56968C34.4277 4.69143 34.3044 4.78997 34.1422 4.78997C24.5991 4.78997 16.8421 12.3446 16.8421 21.6889C16.8421 31.0332 24.5991 38.5878 34.1422 38.5878C34.2214 38.5878 34.2769 38.6335 34.2995 38.6911C34.3102 38.7185 34.3077 38.7354 34.3065 38.7399C34.3062 38.7409 34.3059 38.7419 34.3054 38.7429C34.3049 38.744 34.3042 38.7453 34.3031 38.7468C34.3011 38.7496 34.2948 38.7575 34.2789 38.7683Z"
        stroke="#18FFAC"
      />
    </svg>
  );
}

function Star() {
  return (
    <svg
      width="44"
      height="44"
      viewBox="0 0 44 44"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M31.2321 14.3217L31.2321 14.3217L41.9237 19.1202C44.0254 20.0635 44.0254 23.0004 41.9237 23.9436L31.2321 28.7421L31.2321 28.7421C30.4315 29.1015 29.7857 29.7316 29.4125 30.5209L24.4436 41.0307L24.8957 41.2444L24.4436 41.0307C23.4772 43.0749 20.5227 43.0749 19.5563 41.0307L14.5874 30.5209C14.2143 29.7316 13.5685 29.1015 12.7679 28.7421L12.7679 28.7421L2.0763 23.9436L1.87157 24.3998L2.0763 23.9436C-0.0254324 23.0004 -0.025433 20.0635 2.0763 19.1202L1.87157 18.664L2.0763 19.1202L12.7679 14.3217L12.7679 14.3217C13.5685 13.9624 14.2143 13.3322 14.5874 12.5429L19.5563 2.03309C20.5227 -0.0110287 23.4772 -0.0110279 24.4436 2.03308L29.4125 12.5429C29.7857 13.3322 30.4315 13.9624 31.2321 14.3217Z"
        stroke="#18FFAC"
      />
    </svg>
  );
}

function CrescentAndStar() {
  return (
    <svg
      width="44"
      height="44"
      viewBox="0 0 44 44"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M33.9638 15.7271L33.9638 15.7271L41.3177 17.3876L41.2791 17.8861C41.2791 17.8861 41.2791 17.8861 41.2791 17.8861C43.3926 18.05 44.3054 20.814 42.6439 22.2678C41.6658 23.1236 41.2452 24.4694 41.5463 25.7445L41.5463 25.7445C42.0668 27.947 39.754 29.5948 37.9677 28.4567C36.8713 27.758 35.484 27.758 34.3877 28.4566C32.6014 29.5948 30.2886 27.9471 30.809 25.7445C31.1103 24.4694 30.6895 23.1236 29.7115 22.2678C28.0499 20.814 28.9628 18.05 31.0763 17.8861C32.3659 17.786 33.4798 16.9445 33.9638 15.7271Z"
        stroke="#18FFAC"
      />
      <path
        d="M34.4773 38.7878L34.7565 39.2026L34.4773 38.7879C30.9802 41.1419 26.7333 42.5222 22.1519 42.5222C10.18 42.5222 0.5 33.1015 0.5 21.5112C0.5 9.92072 10.18 0.5 22.1519 0.5C26.7922 0.5 31.0892 1.91597 34.6117 4.32552L34.893 3.91435L34.6117 4.32553C34.6713 4.36627 34.6886 4.40414 34.6953 4.43098C34.7034 4.46379 34.7028 4.51269 34.6783 4.57128C34.6277 4.69247 34.5032 4.79271 34.3373 4.79271C24.7436 4.79271 16.9412 12.3487 16.9412 21.7001C16.9412 31.0515 24.7436 38.6075 34.3373 38.6075C34.4198 38.6075 34.476 38.6549 34.4984 38.7118C34.5091 38.7389 34.5065 38.7554 34.5054 38.7594C34.5053 38.76 34.5051 38.7606 34.5048 38.7612C34.5043 38.7625 34.5035 38.764 34.5021 38.7659C34.5001 38.7688 34.4936 38.7769 34.4773 38.7878Z"
        stroke="#18FFAC"
      />
    </svg>
  );
}

function Reason({ title, children }: PropsWithChildren<{ title: string }>) {
  return (
    <div className="flex max-w-[280px] items-center gap-x-[16px]">
      {children}
      <Text className="line-clamp-2">{title}</Text>
    </div>
  );
}

export function WhyBlock() {
  return (
    <Container>
      <div className="bg-islamic-primary-graphite flex transform-gpu flex-col items-start rounded-[20px] px-[20px] py-[28px] text-white backdrop-blur-sm md:p-[40px] lg:mt-[100px] lg:p-[48px]">
        <Heading>Why Islamic Coin?</Heading>
        <Text isMono className="mt-[12px] md:mt-[20px]">
          Pioneering the Future of Islamic Finance in the Web3 Era
        </Text>
        <div className="mt-[24px] grid w-full grid-cols-1 gap-[24px] md:mt-[44px] md:grid-cols-2 xl:grid-cols-4">
          <Reason title="Maintaining Shariah Guidelines">
            <IslamStar />
          </Reason>
          <Reason title="HAQQ Ecosystem's Prime Currency">
            <DoubleCrescent />
          </Reason>
          <Reason title="Reviving Communities with Sustainable Practices">
            <Star />
          </Reason>
          <Reason title="Islamic Way to Financial Freedom">
            <CrescentAndStar />
          </Reason>
        </div>
      </div>
    </Container>
  );
}
