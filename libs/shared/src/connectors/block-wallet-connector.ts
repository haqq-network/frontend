...
export type BlockWalletConnectorOptions = Pick<
...
export class BlockWalletConnector extends InjectedConnector {
  override readonly id = 'blockWallet';

  protected override shimDisconnectKey = `${this.id}.shimDisconnect`;

  #UNSTABLE_shimOnConnectSelectAccount: BlockWalletConnectorOptions['UNSTABLE_shimOnConnectSelectAccount'];
  ...
  constructor({
    ...
    chains?: Chain[];
    options?: BlockWalletConnectorOptions;
  } = {}) {
    const options = {
      name: 'BlockWallet',
      shimDisconnect: true,
      ...
      getProvider() {
        ...
        const ethereum =
          typeof window !== 'undefined'
            ? (window.ethereum as ExtendedEth)
            : undefined;
        if (ethereum && ethereum.providers) {
          return ethereum.providers.find(getReady);
        }
        return getReady(ethereum as ExtendedEth);
      },
      ...options_,
    };
