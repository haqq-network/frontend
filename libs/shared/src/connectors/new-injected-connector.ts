...
import { hexValue, getAddress } from 'ethers/lib/utils';

type ExtendedEth = Ethereum & {
  isBlockWallet: boolean;
};

export type SuperInjectedConnectorOptions = {
  /** Name of connector */
   ...
   */
  getProvider?: () => ExtendedEth | undefined;
  /**
...
type ConnectorOptions = SuperInjectedConnectorOptions &
  Required<Pick<SuperInjectedConnectorOptions, 'getProvider'>>;
...
> {
  ...
  constructor({
    ...
    chains?: Chain[];
    options?: SuperInjectedConnectorOptions;
  } = {}) {
    const options = {
      shimDisconnect: true,
