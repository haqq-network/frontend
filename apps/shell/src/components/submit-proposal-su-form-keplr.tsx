...
export function CreateProposalFormKeplr() {
  ...
  const { submitProposalSoftwareUpgradeKeplr } = useProposalActions();
  ...
  const handleSubmitProposalKeplr = useCallback(
    async (validatetFormData: CreateProposalForm) => {
      const keplr = await getKeplrWallet();

      if (keplr) {
        try {
          const submitProposalPromise = submitProposalSoftwareUpgradeKeplr(
            validatetFormData,
            keplr,
          );
    ...
    [explorer.cosmos, submitProposalSoftwareUpgradeKeplr, toast],
  );

  const onFormSubmitKeplr: SubmitHandler<CreateProposalForm> = (data) => {
